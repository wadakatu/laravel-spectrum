...
class LargeFormRequestPerformanceTest extends TestCase
{
    ...
    {
        // Arrange - Use MediumFormRequest fixture with 50 fields
        $formRequestClass = MediumFormRequest::class;
        ...
        // Act
        $parameters = $this->analyzer->analyze($formRequestClass);

        $executionTime = microtime(true) - $startTime;
        $memoryUsed = memory_get_usage(true) - $startMemory;

        // Assert - MediumFormRequest has 50 fields
        $this->assertCount(50, $parameters);

        // Verify parameter details
        $stringFieldParam = $this->findParameterByName($parameters, 'string_field_5');
        $this->assertNotNull($stringFieldParam);
        $this->assertEquals('string', $stringFieldParam['type']);
        $this->assertTrue($stringFieldParam['required']);
        $this->assertEquals('String Field 5 Description', $stringFieldParam['description']);
        $this->assertContains('min:3', $stringFieldParam['validation']);
        $this->assertContains('max:255', $stringFieldParam['validation']);

        $integerFieldParam = $this->findParameterByName($parameters, 'integer_field_1');
        $this->assertNotNull($integerFieldParam);
        $this->assertEquals('integer', $integerFieldParam['type']);
        $this->assertEquals('Integer Field 1 Description', $integerFieldParam['description']);
        $this->assertContains('min:0', $integerFieldParam['validation']);
        $this->assertContains('max:1000000', $integerFieldParam['validation']);

        $emailFieldParam = $this->findParameterByName($parameters, 'email_field_2');
        $this->assertNotNull($emailFieldParam);
        $this->assertEquals('string', $emailFieldParam['type']);
        $this->assertEquals('email', $emailFieldParam['format'] ?? null);
        $this->assertEquals('Email Field 2 Description', $emailFieldParam['description']);

        // Performance assertions for 50 fields
    ...
    /**
     * Find parameter by name in the parameters array
     */
