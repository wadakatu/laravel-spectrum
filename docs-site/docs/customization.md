...
use LaravelSpectrum\Contracts\ResponseFormatter;

class GraphQLFormatter implements ResponseFormatter
{
    /**
    ...
    public function format($response): array
    {
        ...
        return [
            ...
            'extensions' => $this->getExtensions($response),
        ];
    }
    
    private function formatData($response): array
    {
    ...
    private function formatErrors($response): array
    {
        $errors = [];
...
```php
namespace App\Spectrum\Plugins;

use LaravelSpectrum\Contracts\Plugin;
use LaravelSpectrum\Events\DocumentationGenerated;
use LaravelSpectrum\Events\RouteAnalyzed;
...
class MetricsPlugin implements Plugin
{
    ...
    public function register(): void
    {
        ...
        app('events')->listen(RouteAnalyzed::class, [$this, 'onRouteAnalyzed']);
        app('events')->listen(DocumentationGenerated::class, [$this, 'onDocumentationGenerated']);
    }
    
    /**
     * Route analysis hook
     */
    public function onRouteAnalyzed(RouteAnalyzed $event): void
    {
        $this->metrics['routes_analyzed'] = ($this->metrics['routes_analyzed'] ?? 0) + 1;
        
        // Calculate complexity
        $complexity = $this->calculateComplexity($event->getRoute());
        if ($complexity > 10) {
            $this->metrics['complex_routes'][] = $event->getRoute()['uri'];
        }
    }
    
    /**
     * Documentation generation completion hook
     */
    public function onDocumentationGenerated(DocumentationGenerated $event): void
    {
        $openapi = $event->getOpenApi();
        
        // Add metrics to OpenAPI
        $openapi['x-metrics'] = $this->metrics;
        
        // Generate report
        $this->generateReport();
    }
    
    private function calculateComplexity(array $route): int
    {
        $complexity = 0;
        
        // Number of parameters
        $complexity += count($route['parameters'] ?? []);
        
        // Number of response types
        $complexity += count($route['responses'] ?? []);
        
        // Nesting depth
        if (isset($route['requestBody']['content'])) {
            $complexity += $this->getSchemaDepth($route['requestBody']['content']);
        }
    ...
    private function generateReport(): void
    {
        $report = [
            ...
            'total_routes' => $this->metrics['routes_analyzed'] ?? 0,
            'complex_routes' => $this->metrics['complex_routes'] ?? [],
            'average_parameters' => $this->calculateAverageParameters(),
        ];
        
        file_put_contents(
            storage_path('app/spectrum/metrics-report.json'),
...
// config/spectrum.php
'plugins' => [
    \App\Spectrum\Plugins\MetricsPlugin::class,
    \App\Spectrum\Plugins\SecurityPlugin::class,
    \App\Spectrum\Plugins\VersioningPlugin::class,
],
```
